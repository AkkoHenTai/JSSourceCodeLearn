...
type PrmiseFn = (resolve: Function, reject: Function) => void;
...
class MyPromise {
    ...
    // ----------------------实例方法开始↓----------------------------//
    then = (fulfilledFn?: Function | undefined | null, rejectedFn?: Function | undefined | null) => {
        // 处理函数，判断是否是函数，如果不是则返回原值
        fulfilledFn = typeof fulfilledFn === 'function' ? fulfilledFn : (value: any) => value;
        rejectedFn = typeof rejectedFn === 'function' ? rejectedFn : (error: any) => { throw error };
    ...
    static race = (promises: any[]) => {
        ...
        return new MyPromise((resolve, reject) => {
            ...
            for (let promise of nPromises) {
                promise.then((res: any) => resolve(res)).catch((res: any) => reject(res));
            }
        });
    };
...
MyPromise.race([promise1, promise2]).then((res: any) => {
